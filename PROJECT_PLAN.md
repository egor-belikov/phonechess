...
- Реализованы профили и логины (опциональный логин, профиль, история партий, replay из истории).
- Включен strict single active client policy (второй одновременный вход отклоняется).
- Рейтинги blitz/rapid ведутся с базой старта `1500` и автопересчетом по результатам.
...
- Добавлен режим «игра с ботом» (без записи в профиль): слабый серверный бот, фиксированная задержка хода.
...
1. Полировка режима игры с ботом: опциональное подключение реального UCI-движка (Skill 0 / LimitStrength) и выбор силы.
2. Наблюдение за партиями (топ-партии в лобби, полноценный режим зрителя в realtime).
3. Турнирный модуль (в конец roadmap): foundation -> Swiss -> KO -> профильная турнирная история/награды.
4. Улучшение reliability деплоя и мониторинга (health/checks, алерты, стабильный CI/CD).
5. Расширение i18n покрытия (полный паритет новых UX-строк во всех локалях).
