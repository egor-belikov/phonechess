...
**Ограничение по устройствам:** приложение рассчитано **только на смартфоны**. Планшеты и десктоп (в т.ч. Telegram Desktop, веб-версия в браузере на компе) не поддерживаются — интерфейс и сценарии заточены под мобильный экран и тач. При открытии не с телефона показывать блокирующий экран с просьбой открыть приложение на телефоне. Проверку делать на клиенте: ширина viewport (например ≤520px) и при наличии — `Telegram.WebApp.platform` (разрешать только `android` и `ios`). В дальнейшем при необходимости можно дублировать проверку на бэкенде по заголовкам/данным от Telegram.
...
- **Ввод хода:**
  - Перетаскивание фигуры (desktop drag & drop + touch drag на мобильных).
  - Либо: нажатие на фигуру → индикаторы легальных полей → нажатие на поле для хода.
- **Премувы (уже реализовано):**
  - можно ставить во время хода соперника;
  - очередь премувов не ограничена;
  - премув исполняется только если легален в новой позиции после хода соперника;
  - если первый премув в очереди стал нелегальным, очищается вся очередь;
  - двойной тап/клик по одной клетке очищает очередь премувов;
  - доска показывает превью-позицию по цепочке премувов + цветные шаги.
...
- **Служебная мета внизу экрана:** версия и время деплоя для проверки актуальности клиента во время тестов.
...
- Если `telegram_id` ещё не в базе — показать **экран регистрации**: ввод **логина** (опционально, можно играть анонимно).
- Правила логина: только `a-z`, `A-Z`, `0-9`, `_`, `-`, `.`; макс. **20 символов**; уникальность по всей базе.
...
## 8. Автоматические турниры

- Отдельные **очереди ожидания** по каждому контролю времени для форматов **Swiss** и **KO**.
- **Swiss:** автозапуск по расписанию (по умолчанию каждые **30 минут**, переменная окружения `TOURNAMENT_INTERVAL_SEC`), если набрано не меньше **8** игроков (`SWISS_MIN_PLAYERS`). Раунды: до `SWISS_MAX_ROUNDS` (по умолчанию 5), ничья в партии — 0.5 очка. Топ-3 получают места в таблице результатов.
- **KO (classic):** минимум **16** участников (`KO_MIN_PLAYERS`), сетка 1/8 → четверть → полуфинал → финал; **mini-match из двух партий** с разными цветами; при равенстве очков — **tie-break** (одна партия, белые у игрока с лучшим посевом). Есть **матч за 3-е место** между проигравшими полуфиналов. Награды **топ-3**.
- Партии турнира **не меняют** рейтинг blitz/rapid в профиле (в отличие от обычных матчей).
- В профиле — **история турниров** (формат, контроль, место, награда).

## 9. Просмотр чужих партий (зрители)
...
## 10. Поведение при закрытии/сворачивании
...
### Текущий рабочий процесс деплоя (факт)

1. Обновить build-метаданные и кэш-теги:
   - `python3 scripts/update_build_meta.py`
2. Залить изменения в репозиторий и на сервер.
3. На сервере (`egorvps`, `/root/my_projects/phonechess`):
   - `docker compose build app`
   - `docker compose up -d app`
4. Проверка:
   - `curl http://127.0.0.1:8000/health`
   - в UI внизу видны актуальные `Версия` и `Деплой`.
...
## 13. Структура репозитория (предложение)
...
3. **UI под телефон:** доска Lichess-like, drag & drop/touch-drag, подсветка ходов и премувов, красный король при шахе, таймер с миллисекундами при < 20 сек, список ходов с временем.
...
- Этапы 1-3 выполнены в рабочем прототипе.
- Реализованы приватные игры через Telegram-бот (bot-mediated invite flow) с комнатой ожидания.
- После приватного матча бот **редактирует** стартовые сообщения у обоих игроков: итог и список ходов в моноширинном блоке (`<pre>`), HTML-формат.
- Реализованы профили и логины (анонимный режим по умолчанию, опциональный логин до **20** символов, профиль, история партий, replay из истории).
- Включен strict single active client policy (второй одновременный вход отклоняется с кодом `4009`).
- Рейтинги blitz/rapid ведутся с базой старта `1500` и автопересчетом по результатам (турнирные партии рейтинг не меняют).
- Добавлены премувы, индикация очереди премувов, пинг под доской и build-info на экране.
- Улучшен mobile input: стабильный tap-select фигуры с фиксацией и отдельной яркостью маркеров для tap/drag.
- Добавлен режим «игра с ботом» (без записи в профиль): серверный UCI Stockfish (слабые настройки), фиксированная задержка хода.
- Добавлен авто-аборт несостоявшейся партии (если оба не сделали первые ходы за 60 секунд) и базовый приватный рематч по взаимному согласию.
- **Турниры:** очереди Swiss/KO по контролям, планировщик автозапуска, Swiss (≥8 игроков), KO classic (≥16, bo2 + tie-break, матч за 3-е, топ-3), история в профиле (`GET /api/tournaments/history`).
- Налажен цикл быстрого деплоя на VPS с Docker Compose.

### Ближайшие приоритеты (актуальный roadmap)

1. Полировка UX турниров: i18n строк, уведомления в Telegram о старте/этапах, защита от оффлайна в сетке KO.
2. Наблюдение за партиями (топ-партии в лобби, полноценный режим зрителя в realtime).
3. Улучшение reliability деплоя и мониторинга (health/checks, алерты, стабильный CI/CD).
4. Расширение i18n покрытия (полный паритет новых UX-строк во всех локалях, в т.ч. турниры и профиль).
